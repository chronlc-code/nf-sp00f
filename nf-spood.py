#! /usr/lib/python-exec/python3.12/python
"""
...
# from smartcard.Exceptions import CardRequestTimeoutExceptionimport time
import os
import getopt
import sys
import string
...
DF_PPSE = [0x32, 0x50, 0x41, 0x59, 0x2E, 0x53, 0x59, 0x53, 0x2E, 0x44, 0x44, 0x46, 0x30, 0x31]
#2PAY.SYS.DDF01 = PPSE = [0x32, 0x50, 0x41, 0x59, 0x2E, 0x53, 0x59, 0x53, 0x2E, 0x44, 0x44, 0x46, 0x30, 0x31]
#SELECTPPSE_APDU = [0x00], [0xA4], [0x04], [0x00], [0x0E], [0x32], [0x50], [0x41], [0x59], [0x2E], [0x53], [0x59], [0x53], [0x2E], [0x44], [0x44], [0x46], [0x30], [0x31], [0x00]
...
TAGS=   {   
    ...
    0x42:['Issuer Identification Number (IIN)',BINARY,ITEM],
    ...
    0x91:['Issuer Authentication Data',BINARY,VALUE],
    ...
    0x5f55:['Issuer Country Code',TEXT,ITEM],
    ...
    0x9f67:['Card Production Life Cycle',BINARY,ITEM],
...
def printMenu():
    ...
    print("\nRFiD AiO TOOLKiT")
    print("\nVer 1.0")
    print("\nCLi - sudo ./nfc2emv.py [options] [PIN]")
    print("\n")
    print("\nMAiN MENU")
    print("\nOPTiONS:\n")
...
def bytesToHex(data):
    data2 = ""
    datalen=len(data)
    data2 = ''.join([f"{x:02x}" for x in data[:datalen]])
    rxData = [''.join(x)for x in zip(data2[::2], data2[1::2])]
    data3 = bytes.fromhex(data2)

    txData = []
    for i in range(0, len(data2), 2):
        txData.append(int(data2[i : i + 2], 16))
    return txData

def get_tag(data,req):
    "return a tag's data if present"
   # req = "".join([f"{x:02x}" for x in req])
  ...
  #      print(tag, " ", req)
        if tag == req:
            ...
            index += taglen + 1
            #print(data[index:index + itemlength], "tegger")
...
def hexbytes(data, block_size=1):
    """
    Converts bytes to a hex string, formatted in blocks.

    Args:
        data: The bytes object to convert.
        block_size: The number of bytes per block in the output string.

    Returns:
        A string representing the hexadecimal representation of the bytes,
        formatted in blocks separated by spaces.
    """
    hex_string = data.hex()
    blocks = [hex_string[i:i + block_size * 2] for i in range(0, len(hex_string), block_size * 2)]
...
def decode_pse(data):
   ...
   # print(data[1])
    #data = []
    #for i in range(0, len(data2), 2):
    #    data.append(int(data2[i : i + 2], 16))
    "decode the main PSE select response"
    Verbose = False
    index= 0
    indent= '   ++++++++   '
    #data = [hex(num) for num in hexData]
    if OutputFiles:
    ...
    if RawOutput:
        ...
        textprint(data)
        return
    print("#### -DATA FOUND- DECODiNG...")
    print("#### -DATA DECODED- LiSTiNG DATA...\n")
    while index < len(data):
        tag = data[index]
        #print("%#02X" % tag)
        #print(f"#{index} - {tag}")
        if tag in TAGS:
            ...
            if tag in TAGS:
                ...
                #tag = bytes.fromhex(data[index])
                hexTag = hex(tag)
                print()
                print(indent + "  Unrecognised TAG:", end="")
                #print(hexTag)
                #print(data[index + taglen + of])
                print("%02x" % data[index + taglen])
                #print(data[index])
        print(f"{indent} {TAGS[tag][0]}", end="")
        if TAGS[tag][2] == VALUE:
            itemlength = 1
            offset = 0
        else:
...
def send_apdu(apdu):
    ...
    response, sw1, sw2 = card.nfc.sendAPDU(apdu)
...
def select_ppse():
    # try to select PSE
#    cla = "00" ins = "A4" p1 = "04" p2 = "00"   lc = [len(ppse)] lc = "0E" data = "325041592E5359532E4444463031" le = "00"
    apdu = SELECT + [len(DF_PPSE)] + DF_PPSE
    apduout = "".join("{:02x}".format(num) for num in apdu)
    response, sw1, sw2 = send_apdu(apduout)
    #response2 = "".join("{:02x}".format(num) for num in response)
    if SW1_RESPONSE_BYTES:
        # there is a PSE
        print("#### -PSE LOCATED-")
        print()
        decode_pse(response)
        check, results, sw1, sw2 = select_aid(aidlist[1][1:])
        success, length, pdol = get_tag(results, 0x9f38)
        pdol = bytesToHex(pdol)
        print(pdol, "pdol")
        pdollist = list() 
        x = 0
    #    hexprint(pdol)
    #    print(results)
        while x < (len(pdol)): 
            ...
            x += 1
        #pdollen = len(pdollist)
        #print("%02x" % pdollist)
        #pdollist = ''.join([f"{x:02x}" for x in pdollist[:pdollen]])    
        print(pdollist, "1")
       # pdollist = bytes.fromhex(pdollist)
        print(hex(pdollist), "2")
...
def select_aid(aid):
    ...
    if check_return(sw1,sw2):
       # if Verbose:
        ...
        return False, response, sw1,sw2

def bruteforce_aids():
    #brute force two digits of AID
    print("### -BRUTEFoRCE AiDS-")
    y= z= 0            
    if BruteforceEMV:
        brute_range= [0xa0]
    else:
...
def decode_ber_tlv_item(data):
    ...
    if data[i] & TLV_LENGTH_MASK:
        ...
        length= int(data[i])
        i += 1
    #print(tag, i + length, data[i:i+length])
...
#        print("    Tag ID: " + card.uid)
    ...
    while 42:
        if card.select("A") or card.select("B"):
            print("#### -CARD FOUND- ")
            print("#### -CARD iD (TAG)- ", card.uid)
            print("#### -LOOKING FOR PSE DATA-")
            select_ppse()
